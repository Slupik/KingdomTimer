 ...
 */
public class TaskTableRow extends TableRow<TaskFxBean> {
    private TaskFxBean lastTask;
    ...
    private ChangeListener<TaskType> backgroundChanger = (observable, oldValue, newValue) -> {
        updateRowColors();
    };

    @Override
    public void updateItem(TaskFxBean item, boolean empty) {
    ...
    private static void setTextColor(TaskTableRow row, TaskType type) {
    ...
    private static void setTextColor(TaskTableRow row, Paint paint) {
        ...
        for(Node node:nodes) {
            TableCell<TaskFxBean, ?> cell = ((TableCell<TaskFxBean, ?>) node);
            cell.setTextFill(paint);
        }
    }

    private static Paint getTextPaint(TaskType type, boolean isDarkLayout) {
        if(isDarkLayout) {
            switch (type) {
                case UNKNOWN: {
                    return Color.RED;
    ...
    private static void setRowColor(TableRow<TaskFxBean> row, TaskType type) {
        row.setBackground(getRowBackground(type,false));
    }

    private static Background getRowBackground(TaskType type, boolean isDarkLayout) {
        Color toFill;
        switch (type) {
            case UNKNOWN: {
                if(isDarkLayout) {
